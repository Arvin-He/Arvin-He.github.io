...
NSIS将所有文件和安装脚本编译成一个可执行文件，以便您的应用程序易于分发。可以创建与Windows 95,98，ME，NT4,2000，XP，2003，Vista，Server 2008,7，Server 2008R2,8，Server 2012，8.1，Server 2012R2和Windows 10兼容的单个安装程序.可以选择三种不同的集成压缩方法（ZLib，BZip2，LZMA）。 LZMA压缩比任何其他常见的压缩方法提供更好的结果。不必使用大型自解压缩存档模块或其他应用程序，压缩支持包含在34 KB开销中。与只能根据文件和注册表项列表生成安装程序的其他系统不同，NSIS具有强大的脚本语言。此脚本语言是为安装程序设计的，并具有帮助您执行许多安装任务的命令。您可以轻松添加自定义逻辑并处理不同的升级，版本检查和更多。脚本语言提供可以在目标系统上使用的命令。从文件夹创建和注册表编辑到文本/二进制文件修改，修改环境变量和系统重新引导等简单功能。通过使用所提供的插件之一，甚至可以直接调用Windows API。可以创建自定义向导页面以获取用户输入或集成配置选项。 NSIS包括一个经典和现代的向导界面，甚至可以创建自己的自定义界面（对话框，字体，背景，图标，文本，复选标记，图像等）。
NSIS编译器具有强大的预处理器。允许将多个项目集成到单个安装程序中或自动生成安装程序构建。还可以生成不同的版本，如精简版和完整版本。完全多语言，在一个安装程序中支持多种语言。有60多个翻译可用，但您也可以创建自己的翻译。 Unicode支持允许更多的语言。 Installer 使用CRC32校验和进行自检 压缩数据大小的开销较小.能够以文本或RTF格式显示许可协议 能够从注册表检测目标目录.易于使用的插件系统（包括用于创建自定义对话框，互联网连接，HTTP下载，文件修补，Win32 API调用等的插件）.Installer 可以大到2GB .自动安装的可选静音模式 一个预处理器，支持定义的符号，宏，条件编译，标准预定义 .具有PHP和汇编元素的编码体验（包括用户变量，堆栈，实际流量控制等）.

### NSIS版本选择
NSIS有unicode版本和GBK版本,选择NSIS时需要注意,如果你想以utf-8打开nsi文件,那么请选择NSIS的unicode版本的.否则选择GBK版本的.否则在编译nsi文件时会报错.

### 脚本文件
要创建一个NSIS安装程序，首先必须编写一个NSIS脚本。 NSIS脚本只是一个具有特殊语法的常规文本文件。可使用任何文本编辑器编辑脚本。建议使用显示行号的文本编辑器，因为NSIS使用行号来指示错误的位置，并警告您错误可能在哪里。还建议支持语法高亮的编辑器。
...
### 变量
使用Var命令声明自己的变量（$ VARNAME）。变量是全局的，可以在任何部分或功能中使用。
声明和使用用户自定义变量：
```
...
### 编译器命令
编译器命令将在您的计算机上的编译时执行。它们可用于条件编译，包括头文件，执行应用程序，更改工作目录等。
最常见的用法是定义。定义是编译时常数,可以定义产品的版本号，并在脚本中使用。例如：
```
...
### 编译器
创建脚本之后，为了创建安装程序，需要做的第二件事就是编译脚本。 MakeNSIS.exe是NSIS编译器。它读取您的脚本，解析它并为您创建一个安装程序。
...
编译器将检查您的脚本，并给您警告或错误。如果发生错误（即需要2个参数，但只有1个参数），则编译器将中止，并显示包括行号的短错误消息。对于非严重错误，编译器将发出警告（即，一个脚本中的两个DirText命令）。如果您的脚本没有错误，编译器将输出一个安装程序供您分发。它也可以在Linux，BSD或Mac OS X服务器上编译Windows安装程序。
